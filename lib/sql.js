...
class Query
{
  ...
  execute()
  {
    if( this.query.table )
    {
      ...
      {
        let executed = await this.connector.select( this.query.table );

        if( !executed.ok ){ executed.error = new SQLError( executed.error.code ).get(); }
  ...
  _get_existing_rows( data )
  {
    return new TimedPromise( async( resolve, reject, remaining_ms ) =>
    {
      ...
      //    else{ resolve({  ok: false, error: table_indexes.error }); } //TODO odkomentovat
        }
  ...
  create_table( execute )
    ...
    if( execute )
    {
      return new TimedPromise( async( resolve, reject, remaining_ms ) =>
      {
        resolve( await SQL( query ).execute());
      });
    }
    else { return query; }
	}
