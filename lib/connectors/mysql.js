...
module.exports = function( config )
{
  return new( function()
  {
    ...
    this.build = function( query )
    {
      ...
      {
        let escape_columns = Abstract_Connector.escape_columns_group( query.columns.columns, query.group_by, query.order, 'MAX' );
        let is_agregated = escape_columns.is_agregated;
        let escaped_columns = escape_columns.columns;
        ...
        querystring += 'SELECT ' + Abstract_Connector.escape_columns( Abstract_Connector.expand_values( escaped_columns, query.columns.data, MYSQL_Connector.escape_value ), MYSQL_Connector.escape_column ) + ( ( query.table && query.table != 'TEMPORARY' ) ? ' FROM ' + Abstract_Connector.escape_columns( query.table, MYSQL_Connector.escape_column ) : '' );
      }
