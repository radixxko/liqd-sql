...
module.exports = function( config, emit )
{
	return new( function()
	{
		...
		function try_query( query, callback, timeout_ms )
		{
			...
			my_connections.query( query, ( err, result ) =>
			{
				let elapsed = process.hrtime(start), remaining_ms = timeout_ms - elapsed[0] * 1000 - Math.ceil( elapsed[1] / 1e6 );

				if( err && connectionLostErrors.includes( err.code ) && remaining_ms > 100 )
				{
					if( my_connected )
					{
					...
					{
						my_offline_queries.push({ deadline: (new Date()).getTime() + remaining_ms, query, callback });
					}
				}
		...
		function connect()
		{
			...
			my_ping_interval = setInterval( ping, 10000 );
			...
			const connectHandler = ( conn ) =>
			{
				...
				if( !my_connected )
				{
					if( my_ping_interval ){ clearInterval( my_ping_interval ); }
					my_ping_interval = null;
					my_connected = true;
					...
					while( offline_query = my_offline_queries.shift() )
					{
