...
module.exports = class Connector
{
	...
	_subquery( table, alias = undefined, database = undefined )
	{
		...
		return new Query({ table, alias, database, connector: this, tables: this.tables });
	}
	...
	showColumns( query )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			...
			{
				query.columns.columns = this.db_connector.unescape_column( query.columns.columns );
				...
				let table = query.table, alias = ( query.table_alias && query.table_alias !== '' ? query.table_alias : null );
				...
				if( all_tables || ( tables_columns && tables_columns.indexOf( word ) !== -1 ) )
				{
					...
					{
						await this.describe_table( table );
					}
				...
				if( query.join && query.join.length )
				{
					for( let i = 0; i < query.join.length; i++ )
					{
						if( query.join[i].table )
						{
							...
							{
								if( !/\s*\(.*\)\s*/.test( query.join[i].table ) )
								{
									let table = query.join[i].table, alias = ( query.join[i].table_alias && query.join[i].table_alias !== '' ? query.join[i].table_alias : null );
									let word = ( alias ? alias : table );
				...
				if( all_tables )
				{
					...
					let used_columns = [];
					let used_columns_splitted = query.columns.columns.split(',');
					if( used_columns_splitted ){ query.columns.columns.split(',').forEach( ( column ) => { if( column.trim() === '*' ){ used_columns.push( new_query_columns.join(', ') ); }else{ used_columns.push( column ); } }); }
	...
	query_after_update( query, indexes, before_update, result  )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			...
			{
				if( result_after && result_after.rows && result_after.rows.length )
				{
					for( let i = 0; i < result_after.rows.length; ++i )
					{
	...
	_create_datum( row, existing_row = null, changed_columns = null )
	{
	...
	{
		let indexes = [], indexes_groups = [], indexes_order = [], indexes_groups_iterators = [], indexes_unique_groups = [];
		...
		if( this.tables[query.table] && this.tables[query.table].indexes )
		{
			...
			if( this.tables[query.table].indexes.unique )
			{
				...
				else for( let i = 0; i < this.tables[query.table].indexes.unique.length; ++i )
				{
					indexes.push( this.tables[query.table].indexes.unique[i].split(/\s*,\s*/) );
				}
			}
	...
	_generate_where_for_indexes( group_indexes, data, data_filter = null )
	{
		...
		for( var indexes of group_indexes )
		{
			...
			for( let i = 0; i < indexes.length; ++i ){ data_indexes.set( indexes[i], [] ); }
			...
			for( let i of data_filter )
			{
				...
				for( let j = 0; j < indexes.length; ++j )
				{
					if( !data[i] || typeof data[i][indexes[j]] === 'undefined' ) { empty = true; break; }
				}
				if(empty){ continue; }
	...
	_get_existing_rows( data, query, options = {} )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			...
			if( this.tables && this.tables.hasOwnProperty( query.table ))
			{
				...
				{
					var existing = await this._get_existing_rows_for_index( data,	index.split(','), indexes[index], rows_index, query.table, query.database, options ).timeout( remaining_ms ).catch( (e, remaining_ms ) => e );
					time +=	existing.time;
	...
	_get_existing_rows_for_index( data, indexes, data_indexes, rows_index, table, database, options = {} )
	{
		var columns = ( options.columns ? options.columns : this._get_all_existing_columns( data, indexes, table ));
		var where = this._generate_where_for_indexes( [ indexes ], data, data_indexes );

		return this._subquery( table, undefined, ( database ? database : undefined ) ).where( where.condition, where.data ).get_all( columns ).then( ( rows ) =>
		{
			if( rows.ok )
			{
				let primary = ( this.tables[table].indexes.primary ? this.tables[table].indexes.primary.split(/\s*,\s*/) : null );
				primary = ( primary && primary.length == 1 ? primary[0] : null );
	...
	duplicate_table( old_name, new_name, data = false )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			this.execute_query( this.db_connector.get_duplicate_table_query( old_name, new_name ), async ( result ) =>
			{
				if( data )
				{
					this.execute_query( this.db_connector.get_duplicate_table_data_query( old_name, new_name ), async ( result ) =>
					{
						if( result.ok )
						{
				...
				else if( result.ok && result.rows.length )
				{
	...
	create_database( database, tables, options )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			let queries = [], created_tables = [];

			queries.push( this.db_connector.create_database_query( database, options ) );
			...
			for( let table in tables )
			{
				if( tables.hasOwnProperty( table ) )
				{
					...
					queries.push( this.create_table_query( tables[table], table, database ));
	...
	drop_database( database, options )
	{
		return this.db_connector.drop_database_query( database, options );
	}
	...
	copy_database( copying_database, database )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			let queries = [];
			...
			if( database.ok )
			{
				queries.push( this.db_connector.create_database_query( new_database ) );
				...
				for( let table in tables )
				{
					if( tables.hasOwnProperty( table ) )
					{
						...
						queries.push( this.create_table_query( tables[table], table, { database: new_database } ));
	...
	database( name = null )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			this.execute_query( this.db_connector.getTablesQuery( name ), async ( tables ) =>
			{
				let databaseTables = {};
	...
	create_table_query( table, alias = null, database = null, options = [] )
	{
		return this.db_connector.create_table( table, alias, database, options );
	}

	drop_table_query( table, database = null, options = [] )
	{
		return this.db_connector.drop_table( table, database, options );
	}
			...
			if( modified_table.columns.hasOwnProperty( column ) )
			{
				...
				if( current_table.columns.hasOwnProperty( originColumnName ) )
				{
					...
					if( modiefiedColumnData !== currentColumnData )
					{
						...
						{
							setValues += this.db_connector.escape_columns( modified_table.columns[ column ], this.db_connector.escape_column ) ;
							temporaryColumns[ column ] =  'tmp_' + column;
							modifiedColumn = this.modify_column( column, modiefiedColumnData, previousColumn, setValues );
						}
						else
	...
	compare_indexes( newIndex, oldIndex, columns, tmp_columns, table )
	{
		...
		indexes.forEach( type =>
		{
			...
			if( newIndex && newIndex[ type ] )
			{
				if( !Array.isArray(newIndex[ type ]) ){ newIndex[ type ] = [newIndex[ type ]]; }
			...
			if( oldIndex && oldIndex[ type ] && Object.keys( oldIndex[ type ] ).length > 0 )
			{
				if( newIndex && newIndex[ type ] && newIndex[ type ].length > 0 )
				{
					if( !primary && Array.isArray( oldIndex[ type ] ) )
					{
