...
module.exports = class Connector
{
			...
			if( status === 'connected' )
			{
				...
				while( offline_query = this.offline_queries.shift() )
				{
					if( offline_query.deadline > now + 100 )
					{
						...
						this.execute_query( offline_query.query, offline_query.callback, offline_query.indexes, offline_query.columns );
					}
				}
			}
	...
	execute_query( query, callback, indexes )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			...
			this.db_connector.execute( query, result =>
			{
				...
				if( result.connector_error && result.connector_error.reconnect && remaining_ms > 100 )
				{
					...
					{
						this.offline_queries.push({ deadline: (new Date()).getTime() + remaining_ms, query, callback, indexes });
					}
				}
				else
				{
	...
	_get_existing_rows( data, query )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			if( !this.tables ){ await this.describe_table( query.table ); }
	...
	describe_columns( table, option = null )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			this.execute_query( this.db_connector.getColumnsQuery( table ), async ( result ) =>
			{
				...
				{
					result.ok = false;
					result.error = result.error || { code: 'UNDEFINED_TABLE' };
				}
		...
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			...
			if( database && tables )
			{
				...
				for( let table in tables )
				{
					...
					{
						if( options.dropTables ){ queries.push( this.drop_table_query( table )); }

						queries.push( this.create_table_query( tables[table], table ));

						if( defaultRows.hasOwnProperty( table ) )
						{
	...
	modify_database( modified_tables, defaultRowsPath = null, options = [] )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			...
			if( database.ok )
			{
				...
				for( let oldTableName in tables )
				{
					if( tables.hasOwnProperty( oldTableName ) && !modified_tables.hasOwnProperty( oldTableName ) )
					{
						...
						if(drop_table && options.includes( 'drop_table' )){ alterTables.push( this.drop_table_query( oldTableName )); }
					}
				}
	...
	database()
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			...
			{
				//TODO mssql test need
				let databaseTables = {};
				if( tables.rows && tables.rows.length > 0 )
				{
					tables.rows.forEach( async ( table ) => {
						for( let info in tables.rows[ i ] )
						{
							if( tables.rows[ i ].hasOwnProperty( info ) )
							{
								...
								if( extracted.ok ) { databaseTables[ described.name ] = described.table; }
							}
						}
					});
						...
						if( !primary )
						{
							if( Array.isArray( oldIndex[ type ] ) )
							{
								oldIndex[ type ].forEach( old_index => {
									let exist = false;
									newIndex[ type ].forEach( new_index => { if( old_index && !primary && new_index && old_index === new_index ){ exist = true; break; } });
							...
							else
							{
								for( let indexName in oldIndex[ type ] )
								{
									...
									{
										let exist = false;
										newIndex[ type ].forEach( new_index => { if( oldIndex[ type ].hasOwnProperty(indexName) && !primary && new_index && oldIndex[ type ][ indexName ] === new_index ){ exist = true; break; }});
