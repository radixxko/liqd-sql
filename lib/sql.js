...
class Query
{
  ...
  union( union )
  {
    if( union )
    {
      if( !this.query.union )
      {
        ...
        if( this.query.table ){ this.query.union.push( this.query.table ); }
      }
  ...
  execute( )
  {
    ...
    else{ return { ok : false, error: 'empty_query' }; }
  }
  ...
  get_query( columns = '*', data = null, alias = null )
  {
    ...
    else{ return { ok : false, error: 'undefined_table' }; }
  }
  ...
  set( data )
  {
    ...
    {
      return new TimedPromise(( resolve ) => { resolve( { ok: false, error: 'undefined_table', affected_rows: 0, changed_rows: 0, inserted_id: null, inserted_ids: [], changed_id: null, changed_ids: [], row: null, rows: [] } ) });
    }
  }
      ...
      if( set )
      {
        ...
        if( data && Array.isArray(data) )
        {
          if( !this.tables )
          {
            ...
            if( result.ok ) { this.tables = { [this.query.table] : { indexes: result.indexes } }; }
    ...
    return new TimedPromise( async( resolve, reject, remaining_ms ) =>
    {
      ...
      if( !this.tables )
      {
        ...
        if( table_columns.ok )
        {
          ...
          else{ resolve({  ok: false, error: table_indexes.error }); }
        }
        else{ resolve({  ok: false, error: table_columns.error }); }
      }
      ...
      if( this.tables )
      {
        ...
        for( let index in indexes )
        {
          ...
          if( existing.error )
          {
            resolve({  ok: false, error: existing.error });
          }
        }

        resolve({ ok: true, rows: rows_index, time: time });
      }
      else{ resolve({  ok: false, error: 'empty_table_info' }); }
    });
  }
