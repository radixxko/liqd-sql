        ...
        return this.select( 'SHOW FULL COLUMNS FROM ' + this.escape_column( table )).then( async ( columnsData ) =>
        {
          ...
          if( columnsData.rows && columnsData.rows.length > 0 )
          {
            for( let k = 0; k < columnsData.rows.length; k++ )
            {
              ...
              if( columnsData.rows[ k ]['Collation'] === 'utf8mb4_slovak_ci' )
              {
                columns[ columnName ].multibyte = true;
              }
              else if( columnsData.rows[ k ]['Collation'] === 'utf8mb4_bin' )
              {
                columns[ columnName ].multibyte_bin = true;
              }
            }
