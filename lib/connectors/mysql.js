	...
	return new( function()
	{
		...
		this.build = function( query )
		{
			...
			if( query.operation === 'select' )
			{
				...
				querystring += 'SELECT ' + escaped_columns + ( ( query.table && query.table != 'TEMPORARY' ) ? ' FROM ' + Abstract_Connector.escape_columns( query.table, MYSQL_Connector.escape_column, MYSQL_Connector.transform_function ) : '' );
			}
			else if( query.operation === 'insert' )  // todo pozriet na insert ak ma unique, ale default hodnota je prazdna
			{
				querystring += 'INSERT '+( query.options && query.options.indexOf('ignore') !== -1  ? 'IGNORE' : '' )+' INTO ' + Abstract_Connector.escape_columns( query.table, MYSQL_Connector.escape_column, MYSQL_Connector.transform_function ) + ' (' + Abstract_Connector.expand( query.columns, MYSQL_Connector.escape_column ) + ') VALUES ';
			...
			{
				querystring += 'UPDATE ' + Abstract_Connector.escape_columns( query.table, MYSQL_Connector.escape_column, MYSQL_Connector.transform_function );
			}
			else if( query.operation === 'delete' )
			{
				querystring += 'DELETE FROM ' + Abstract_Connector.escape_columns( query.table, MYSQL_Connector.escape_column, MYSQL_Connector.transform_function );
			}
			...
			if( query.join )
			{
				for( let i = 0; i < query.join.length; ++i )
				{
					...
					{
						querystring += Abstract_Connector.escape_columns( ( query.join[i].type == 'inner' ? ' INNER' : ' LEFT' ) + ' JOIN ' + query.join[i].table + ' ON ' + Abstract_Connector.expand_values( query.join[i].condition, query.join[i].data, MYSQL_Connector.escape_value, MYSQL_Connector.escape_column ), MYSQL_Connector.escape_column, MYSQL_Connector.transform_function );
					}
				}
			}
		...
		this.execute = function( query, callback, indexes = null, columns = null )
		{
		...
		this.create_table = function( table, name = null, options = [] )
		{
			let columns = [], indexes = [];
			let querystring = 'CREATE TABLE ' + ( options.includes('unsafe') ? 'IF NOT EXISTS ' : '' ) + this.escape_column( name );
		...
		this.drop_table = function( table, options = [] )
		{
			let querystring = 'DROP TABLE ' + ( options.includes('unsafe') ? 'IF EXISTS ' : '' ) + this.escape_column( table ) + ';';
			return querystring;
		}
