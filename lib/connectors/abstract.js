...
module.exports =
{
  ...
  escape_columns_group: function( columns, group_by, order_by, escape )
  {
    ...
    {
      let columns_in_group = [];

      if( typeof group_by !== 'undefined' && group_by.hasOwnProperty('condition') && typeof group_by.condition === 'string' && group_by.condition.length )
      {
        group_by.condition.split(',').forEach( ( part ) => { if(part){ columns_in_group.push(part.trim()); } });
      }

      let word = '', alias_word = '', isAlias = false, delimiters = ' \t\r\n,=+-!<>*/%&|^~:',
        keywords = [ 'IN', 'AND', 'OR', 'DIV', 'MOD', 'SEPARATOR', 'DISTINCT', 'BINARY', 'COLLATE', 'IS', 'LIKE', 'ORDER', 'LIMIT', 'SELECT', 'WHERE', 'ORDER', 'BY', 'GROUP', 'UPDATE', 'INSERT', 'JOIN', 'INNER', 'LEFT', 'RIGHT', 'OUTER', 'TOP', 'FROM', 'SET', 'INTO', 'NOT', 'IS', 'XOR', 'NOT', 'ON', 'UNION', 'CASE', 'WHEN', 'ELSE', 'THEN', 'END', 'CURRENT_TIMESTAMP', 'NULL', 'AS', 'HAVING', 'MINUTE', 'NOW', 'INTERVAL', 'ASC', 'DESC', 'TRUE', 'FALSE', 'BETWEEN', 'MONTH', 'DUAL' ],
        uppercase_keywords = [ 'END', 'MAX', 'MIN', 'AVG', 'SUM', 'COUNT' ],
        aggregate_keywords = [ 'MAX', 'MIN', 'AVG', 'SUM', 'COUNT' ],
      ...
      for( let i = 0; i <= columns.length; ++i )
      {
        ...
        {
          if( bracket_count || aggregate_keywords.includes( ( uppercase_keywords.includes( word.toUpperCase() ) ? word : word.toUpperCase() ) ) )
          {
            is_agregated = true;
            dont_agregate = true;
          }
          else if( !dont_agregate && !agregated && columns_in_group.length && columns_in_group.indexOf( word ) === -1 && word != '.' && ( i <= word.length || columns[i-word.length-1] != ':' ) && columns[i] != '(' &&  isNaN(word) && !keywords.includes( ( uppercase_keywords.includes( word.toUpperCase() ) ? word : word.toUpperCase() ) ) )
          {
        ...
        if( agregated && !bracket_count && ( isOf( agregate_delimiters, columns[i] ) || !columns[i] ) && alias_word && !dont_agregate && !bracketed )
        {
    ...
    return { columns: columns, is_agregated: is_agregated };
  },
