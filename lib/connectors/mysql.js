...
module.exports = function( config, emit )
{
  return new( function()
  {
    ...
    {
      let columnRE = /^((`[^`]+`\.)*(`[^`]+`))$/m;
      let columnsRE = /((AVG|BIT_AND|BIT_OR|BIT_XOR|CHECKSUM_AGG|COUNT|COUNT_BIG|GROUP_CONCAT|GROUPING|GROUPING_ID|JSON_ARRAYAGG|JSON_OBJECTAGG|MAX|MIN|STD|STDDEV|STDDEV_POP|STDDEV_SAMP|STRING_AGG|SUM|VAR|VARP|VAR_POP|VAR_SAMP|VARIANCE)\s*\((\s*DISTINCT(\s*\(|\s+){0,1}){0,1}|([\)0-9'"`]{0,1}))\s*((`[^`]+`\.)*`[^`]+`)/gm, column, lastOffset = null;
      let stringsRE = /('[^']*'|"[^"]*")/gm;

      return columns.replace( columnsRE, ( match, _1, aggregated, _2, _3, alias, column, _4, offset ) =>
      {
