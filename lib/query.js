...
module.exports = class Query
{
	...
	async preset( data, columns = null )
	{
		if( this.query.table )
		{
			if( data && ( Array.isArray( data ) ? data.length : typeof data === 'object' ) )
			{
				...
				columns = ( columns ? columns : ( this.tables[ this.query.table ] ? Object.keys( this.tables[ this.query.table ].columns ) : [] ));
				...
				return this.connector._get_existing_rows( data, this.query, { columns } ).then( async( existing ) =>
				{
					for( let i = 0; i < data.length; ++i )
					{
						for( let k = 0; k < columns.length; k++ )
						{
							...
							{
								let default_value = ( this.tables[ this.query.table ].columns[ columns[k] ].hasOwnProperty( 'default' ) ? this.tables[ this.query.table ].columns[ columns[k] ].default : '' );

								if( default_value === 'NULL' || default_value === 'null' ){ data[i][ columns[k] ] = null; }
								else if( !isNaN( parseFloat( default_value ))){ data[i][ columns[k] ] = parseFloat( default_value ); }
								else { data[i][ columns[k] ] = ''; }
							}
						}
					}
