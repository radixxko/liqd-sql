...
module.exports = function( config, emit )
{
	return new( function()
	{
		...
		//this.ping_query = function() //TODO hotovo?

		async function connect()
		{
			let options = JSON.parse(JSON.stringify( config ));
			oracle_sql = require( 'oracledb');
			oracle_sql.fetchAsString = [ oracle_sql.CLOB ];
		...
		this.transform_function = function( transform, position )
		{
			if( transform.hasOwnProperty( position ) && typeof transform[position] === 'object' && transform[position].name )
			{
				...
				{
					if( transform.hasOwnProperty( position - 1 ) && typeof transform[ position - 1 ] === 'object' && transform[ position - 1 ].name ){ return transform[position].values; }
					else
					{
						let label = ' CASE';
		...
		this.build = function( query, auto_increment = false )
		{
			...
			{
				querystring += 'INSERT ALL ';

				for( let i = 0; i < query.data.length; ++i )
				{
					querystring += ' INTO ' + Abstract_Connector.escape_columns( query.table, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function ) + ' (' + Abstract_Connector.expand( query.columns, ORACLE_Connector.escape_column ) + ') VALUES ';

					querystring += '(';

					for( let j = 0; j < query.columns.length; ++j )
					{
						querystring += ( j === 0 ? '' : ',' ) + ( typeof query.data[i][query.columns[j]] !== 'undefined' ? ORACLE_Connector.escape_value(query.data[i][query.columns[j]]) : null );
					}
			...
			if( query.set )
			{
				...
				else if( query.data && Array.isArray(query.data) )
				{
					...
					{
						for( let t_column in query.tables[ query.table ] ){ if( query.tables[ query.table ][ t_column ].type === 'TEXT' ){ clob_columns.push(t_column); } }
					}
					//
			...
			if( query.get_id )
			{
				//querystring += ' RETURNING '+ query.get_id +' INTO :id ';  //trigger for each row return  ideciek spravim
			}

			querystring = querystring.replace(/(!=\s+\'\')/i, 'IS NOT NULL' );
			querystring = querystring.replace(/(>=\s+\'\')/i, '>= '+this.escape_value( ' ' ) );
			querystring = querystring.replace(/(<=\s+\'\')/i, '<= '+this.escape_value( 'ZZZZZZZZZZZZZZZZZZZZZ' ) );
			querystring = querystring.replace(/(=\s+\'\')/i, 'IS NULL' );  //TODO < > najmensi mozny string
			querystring = querystring.replace(/(\'\')/i, 'NULL' );  //TODO < > najmensi mozny string
		...
		this.execute = function( query, callback, indexes = null, auto_increment = null )
		{
			...
			if( typeof query == 'object' )
			{
				...
				if( operation === 'insert' )
				{
				//	query += '; ' + Abstract_Connector.escape_columns( 'SELECT SCOPE_IDENTITY() AS id', ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
				}
			}
			...
			return new TimedPromise( async ( resolve, reject, remaining_ms ) =>
			{
				...
				if( oracle_connections )
				{
					await oracle_connections.getConnection( async (err, conn) =>
					{
						if( !err )
						{
							await conn.execute( query, {}, { outFormat: oracle_sql.OBJECT, autoCommit: true } , BIND_IF_FLOW( async (err, rows) =>
							{
								conn.release( function(err){});
								...
								else
								{
									...
									result.changed_rows = rows.changedRows || result.affected_rows ;
		...
		this.create_table = function( table, name )
		{
			...
			{
				if( type !== 'index' && table.indexes.hasOwnProperty(type) && table.indexes[type] && table.indexes[type].length )
				{
