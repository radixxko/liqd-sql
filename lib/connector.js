...
module.exports = class Connector
{
	...
	_get_existing_rows( data, query )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			...
			if( this.tables && this.tables.hasOwnProperty( query.table ))
			{
				...
				{
					var existing = await this._get_existing_rows_for_index( data,	index.split(','), indexes[index], rows_index, query.table, query.database ).timeout( remaining_ms ).catch( (e, remaining_ms ) => e );
					time +=	existing.time;
	...
	_get_existing_rows_for_index( data, indexes, data_indexes, rows_index, table, database )
	{
