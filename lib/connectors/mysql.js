  ...
  return new( function()
  {
    ...
    function checkConnect( oneTime = false )
    {
      ...
      if( my_connections )
      {
        ...
        {
          if( err && err.code &&  ['PROTOCOL_CONNECTION_LOST', 'ECONNREFUSED', 'ECONNRESET'].includes( err.code ) )
          {
            my_connected = false;
...
*/
    this.select = function( query, callback )
    {
      ...
      return new TimedPromise( async ( resolve, reject, remaining_ms ) =>
      {
        ...
        my_connections.query( query, ( err, rows ) =>
        {
          ...
          if( err )
          {
            if( ['PROTOCOL_CONNECTION_LOST', 'ECONNREFUSED', 'ECONNRESET'].includes( err.code ) )
            {
    ...
    this.describe_table = function( table )
    {
      return new TimedPromise( async ( resolve, reject, remaining_ms ) =>
      {
