...
module.exports =
{
  ...
  get_used_columns: function( query, alias = null, used_columns )
  {
	...
	tabulator: function( count, string )
  {
    if( !string ){ let string = ''; }
    if( count > 0 )
    {
      string = this.tabulator( --count, string );
    }

    return string + '\t';
  },

	niceFormat: function( string )
  {
    if( !string ){ return string; }

    let formating = string.replace(/[{}]/g, '').replace(/"/g, '\'');
	...
	times: function()
  {
    ...
    let hour = (now.getUTCHours() < 10 ? '0'+now.getUTCHours() : now.getUTCHours());
    let minutes = (now.getUTCMinutes() < 10 ? '0'+now.getUTCMinutes() : now.getUTCMinutes());
    let seconds = (now.getUTCSeconds() < 10 ? '0'+now.getUTCSeconds() : now.getUTCSeconds());
