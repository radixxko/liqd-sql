...
module.exports =
{
  ...
  {
    if( ( typeof group_by === 'undefined' && typeof order_by !== 'undefined' ) )
    {
    //console.log( 'ESCAPE ERROR', columns, group_by, order_by );
    }
    ...
    if(  typeof columns === 'string' && typeof group_by !== 'undefined' && group_by.hasOwnProperty('condition') && typeof group_by.condition === 'string' && group_by.condition.length )
    {
      ...
      for( let i = 0; i <= columns.length; ++i )
      {
        ...
        else if( word )
        {
          ...
          else if( !dont_agregate && !agregated && columns_in_group.indexOf( word ) === -1 && word != '.' && ( i <= word.length || columns[i-word.length-1] != ':' ) && columns[i] != '(' &&  isNaN(word) && !keywords.includes( ( uppercase_keywords.includes( word.toUpperCase() ) ? word : word.toUpperCase() ) ) )
          {
      ...
      //  console.log('ALIAS', word,agregated , !bracket_count , isOf( agregate_delimiters, columns[i] ), !columns[i+1]  , alias_word , !dont_agregate , !bracketed );
        if( agregated && !bracket_count && ( isOf( agregate_delimiters, columns[i] ) || !columns[i+1] ) && alias_word && !dont_agregate && !bracketed )
        {
