...
it( 'Create', async() =>
{
	await SQL.query('users_table').drop_table( true );
	await SQL.query({
		columns :
		{
	...
	}, 'users_table' ).create_table( true );
}).timeout(100000);

it( 'Insert', async() =>
{
	insert = await SQL.query( 'users_table' ).insert( { name: 'john' } );
	assert.ok( insert.affected_rows, 'Insert failed ' + JSON.stringify( insert, null, '  ' ) );
}).timeout(100000);

it( 'Select', async() =>
{
	select = await SQL.query( 'users_table' ).where( 'id = :?', 1 ).get( '*' );
	assert.ok( select.ok && select.row && select.row.name === 'john', 'Select failed ' + JSON.stringify( select, null, '  ' ) );
}).timeout(100000);

it( 'Delete', async() =>
{
	delete_row = await SQL.query( 'users_table' ).where( 'id = :?', 1 ).delete();
