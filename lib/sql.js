...
class Query
{
  constructor( table, connector, tables )
  {
  ...
  union( union, alias = null )
  {
    if( typeof alias === 'string' ) { return '( ' + union.join( ' UNION ' ) + ' ) ' + alias ; }
    else { return union.join( ' UNION ' ); }
  }
  ...
  set( data )
  {
    ...
    {
        return new Promise(( resolve ) => { resolve( { ok: true, error: null, affected_rows: 0, changed_rows: 0, inserted_id: null, inserted_ids: [], changed_id: null, changed_ids: [], row: null, rows: [] } ) });
    }
  }
  ...
  update( set, data = null ) // TODO v set metode ak stlpec pre data nie je definovany tak dat len nazonv stlpca WHEN condition THEN COLUMN - poskusat ci pojde vsade
  {
    /* input
        1) a = :a, b = :b		{ a: 1, b: 2 }
...
const SQL = module.exports = function( config )
{
  ...
  {
    return new Query( table, connector, tables );
  }
}
