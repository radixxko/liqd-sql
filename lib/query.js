...
module.exports = class Query
{
	constructor( query )
	{
		...
		this.tables = query.tables;
			...
			else if( typeof this.query.table === 'object' && Array.isArray( this.query.table ) )
			{
				let dual_union = [];
	...
	copy_database( database, user_options = null )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			if( database && typeof database === 'string' )
			{
				resolve( { ok: true, create: await this.connector.copy_database( database )});
			}
			else
			{
				resolve({ ok: false, error: [ 'database' ] });
			}
		});
	}

	create_database( database, tables, user_options = null )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			if( database && typeof database === 'string' )
			{
				...
				if( tables && typeof tables === 'object' && !Array.isArray( tables ) )
				{
					...
					{
						if( user_options.result_type && [ 'string', 'array' ].includes( user_options.result_type ) ){ options.result_type = user_options.result_type; }
	...
	modify_database( tables, user_options = null )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			if( tables && Array.isArray( tables ) )
	...
	{
		let set_start_time = process.hrtime();
		let query = this.connector.create_table_query( this.query.table, this.query.alias, this.query.database, options );
	...
	{
		let set_start_time = process.hrtime();
		let query = this.connector.drop_table_query( this.query.table, this.query.database, options );
