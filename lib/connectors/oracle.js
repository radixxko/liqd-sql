...
module.exports = function( config, emit )
{
	return new( function()
	{
		...
		function aggregate_columns( columns, aggregator = 'MAX' )
		{
			...
			let aggregationRE = /((AVG|BIT_AND|BIT_OR|BIT_XOR|CHECKSUM_AGG|COUNT|COUNT_BIG|GROUP_CONCAT|GROUPING|GROUPING_ID|JSON_ARRAYAGG|JSON_OBJECTAGG|MAX|MIN|STD|STDDEV|STDDEV_POP|STDDEV_SAMP|STRING_AGG|SUM|VAR|VARP|VAR_POP|VAR_SAMP|VARIANCE)\s*\((\s*DISTINCT(\s*\(|\s+){0,1}){0,1}|([\)0-9'""]{0,1}|NULL))\s*?(("[^"]+"\.)*"[^"]+")/gm;
			let stringsRE = /('[^']*'|"[^"]*")/gm;
			...
			return columns.replace( aggregationRE, ( match, _1, aggregated, distinct, _3, alias, column, _4, offset ) =>
			{
				...
				if( aggregate )
				{
					if( !( lastAggregatedDistinct && columnsRE.test( columns.substring( lastAggregatedDistinct, lastOffset ))) )
					{
						...
						let tail = columns.substring( offset + match.length );
						match = match.substring( 0, match.length - column.length ) + aggregator + '(' + column + ')';

						if( !tail || /^\s*,/.test(tail) )
						{
							let parentheses = columns.substring( offset + match.length ).replace( stringsRE, '' ).replace(/[^\(\)]/g, '');
			...
			if( transform.hasOwnProperty( position ) && typeof transform[position] === 'object' && transform[position].name )
			{
				if( transform[position].name.toUpperCase() === 'CONCAT' )
				{
					...
					if( transform[position].values.length )
					{
						...
						label = label.replace(/(DISTINCT\s+)/i, '' );
					}
			...
			if( typeof value === 'string' )
			{
				...
				else
				{
					if (value.indexOf('&__escaped__:') === 0) {
						return value.substring('&__escaped__:'.length);
					}
		...
		this.build = function( query, auto_increment = false )
		{
			...
			if( query.operation === 'select' )
			{
				...
				querystring += 'SELECT '
				querystring += escaped_columns + ( ( query.table && !['DUAL','TEMPORARY'].includes(query.table) ) ? ' FROM ' + Abstract_Connector.escape_columns( ( !/\s*\(.*\)\s*/.test( query.table ) && query.database ? query.database + '.' : '' ) + query.table + ( query.table_alias ? ' ' + query.table_alias : '' ), ORACLE_Connector.escape_column, null ) : '' );
			}
			else if( query.operation === 'insert' && query.options && query.options.indexOf('ignore') !== -1 )  // todo pozriet na insert ak ma unique, ale default hodnota je prazdna
			{
				...
				querystring += 'INSERT INTO ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function ) + ' (' + Abstract_Connector.expand( queryColumn, ORACLE_Connector.escape_column ) + ')';
				querystring += 'SELECT * FROM (';

				let not_exist_select = ' SELECT '+ Abstract_Connector.expand( queryColumn, ORACLE_Connector.escape_column ) + ' FROM ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function ) + ' WHERE ';
				for( let i = 0; i < query.data.length; ++i )
				{
					...
					for( let j = 0; j < queryColumn.length; ++j )
					{
						if( i === 0 )
						{
							not_exist_select += ( j === 0 ? '' : ' AND ' ) + ' ' + Abstract_Connector.escape_columns( 'ins_union.' + queryColumn[j], ORACLE_Connector.escape_column, ORACLE_Connector.transform_function ) + ' = ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table + '.' + queryColumn[j], ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
						}
			...
			else if( query.operation === 'insert' )  // todo pozriet na insert ak ma unique, ale default hodnota je prazdna
			{
				...
				querystring += 'INSERT INTO ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function ) + ' (' + Abstract_Connector.expand( query.columns, ORACLE_Connector.escape_column ) + ') VALUES ';
			...
			{
				querystring += 'UPDATE ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table + ( query.table_alias ? ' ' + query.table_alias : '' ), ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
			}
			else if( query.operation === 'delete' )
			{
				querystring += 'DELETE FROM ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
			}
			else if( query.operation === 'truncate' )
			{
				querystring += 'TRUNCATE TABLE ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
			}
			...
			if( query.join )
			{
				for( let i = 0; i < query.join.length; ++i )
				{
					...
					{
						querystring += Abstract_Connector.escape_columns( ( query.join[i].type == 'inner' ? ' INNER' : ' LEFT' ) + ' JOIN ' + query.join[i].table + ( query.join[i].table_alias ? ' ' + query.join[i].table_alias : '' ) + ' ON ' + Abstract_Connector.expand_values( query.join[i].condition, query.join[i].data, ORACLE_Connector.escape_value, ORACLE_Connector.escape_column ), ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
					}
				}
			}
			...
			if( query.set )
			{
				...
				else if( query.data && Array.isArray(query.data) )
				{
					...
					let specific_columns_types = {};

					if( query.tables && query.tables.hasOwnProperty( query.table ) )
					{
						for( let t_column in query.tables[ query.table ].columns )
						{
							if( query.tables[ query.table ].columns.hasOwnProperty( t_column ) )
							{
								let [ type ] = query.tables[ query.table ].columns[ t_column ].type.split(/[\s,:]+/);

								if( [ 'TEXT' ].includes( type ) ){ specific_columns_types[ t_column ] = { type: 'clob' }; }
								else if(  [ 'DECIMAL' ].includes( type ) )
								{
									let [ ,,precision ] = query.tables[ query.table ].columns[ t_column ].type.split(/[\s,:]+/);
									specific_columns_types[ t_column ] = { type: 'decimal', precision: ( !isNaN( precision ) ? precision : 0 ) };
								}
								else if(  [ 'BIGINT', 'INT', 'TINYINT' ].includes( type ) ){ specific_columns_types[ t_column ] = { type: 'number' }; }
							}
						}
					}
					...
					for( let i = 0; i < columns.length; ++i )
					{
						if( columns[i].substring(0,2) == '__' && columns[i].substring(-2) == '__' ){ continue; }
						...
						for( let j = 0; j < query.data.length; ++j )
						{
							if( typeof query.data[j][columns[i]] !== 'undefined' )
							{
								...
								if( specific_columns_types.hasOwnProperty( columns[i] ) )
								{
									if( specific_columns_types[ columns[i] ].type === 'clob' )
									{
										set += ' THEN to_clob(' + ORACLE_Connector.escape_value(query.data[j][columns[i]])+')';
									}
									else if( specific_columns_types[ columns[i] ].type === 'decimal' )
									{
										let value = query.data[j][columns[i]];
										...
										if( value )
										{
											...
											if( value )
											{
												...
												value = value.toFixed(specific_columns_types[ columns[i] ].precision);
											}
										}

										set += ' THEN ' + value;
									}
									else if( specific_columns_types[ columns[i] ].type === 'number' )
									{
										let value = query.data[j][columns[i]];
										...
										{
											value = value.trim();
											if( value ){ value = Math.floor( parseFloat( value )); }
										}
			...
			if( !query.offset && query.limit && query.operation !== 'insert' )
			{
				querystring = 'SELECT * FROM ('+querystring+') WHERE ROWNUM <= '+this.escape_value(query.limit);
			}
			else if( query.offset && query.limit && query.operation !== 'insert' )
			{
			...
			else if( query.offset && query.operation !== 'insert' )
			{
		...
		this.build_wo_values = function( query, auto_increment = false )
		{
			let querystring = '';
				...
				for( let union of query.union )
				{
						...
						for(let i = 0; i < union.length; i++)
						{
							if( typeof union[i] === 'object' )
							{
								...
								if( used_columns.length )
								{
									for( let p = 0; p < used_columns.length; p++ )
									{
										dual_data.push( ( union[i].hasOwnProperty( used_columns[p] ) ? ':value' : 'NULL' ) + ' ' + Abstract_Connector.escape_columns( used_columns[p], ORACLE_Connector.escape_column, ORACLE_Connector.transform_function)  );
									}
								}
								else
								{
									for( let column in union[i] )
									{
										if( union[i].hasOwnProperty( column ) ){ dual_data.push( ':value ' + Abstract_Connector.escape_columns( column, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function)  ); }
									}
								}
					...
					else if( typeof union === 'object' )
					{
						...
						if( used_columns.length )
						{
							for( let p = 0; p < used_columns.length; p++ )
							{
								dual_data.push( ( union.hasOwnProperty( used_columns[p] ) ? ':value' : 'NULL' ) + ' ' + Abstract_Connector.escape_columns( used_columns[p], ORACLE_Connector.escape_column, ORACLE_Connector.transform_function)  );
							}
						}
						else
						{
							for( let column in union )
							{
								if( union.hasOwnProperty( column ) ){ dual_data.push( ':value ' + Abstract_Connector.escape_columns( column, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function)  ); }
							}
						}
			...
			if( query.operation === 'select' )
			{
				...
				let escaped_columns = Abstract_Connector.escape_columns( query.columns.columns, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
			...
			{
				querystring += 'INSERT INTO ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function ) + ' (' + Abstract_Connector.expand( query.columns, ORACLE_Connector.escape_column ) + ') VALUES :values';
			}
			else if( query.operation === 'update' )
			{
				querystring += 'UPDATE ' + Abstract_Connector.escape_columns( ( query.database ? query.database + '.' : '' ) + query.table + ( query.table_alias ? ' ' + query.table_alias : '' ), ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
			...
			if( query.join )
			{
				for( let i = 0; i < query.join.length; ++i )
				{
					if( typeof query.join[i].table === 'object' && query.join[i].table )
					{
						...
						query.join[i].table.operation = 'select';
						let subquery = this.build( query.join[i].table );
						querystring += Abstract_Connector.escape_columns( ( query.join[i].type == 'inner' ? ' INNER' : ' LEFT' ) + ' JOIN (' + subquery + ') ' + query.join[i].table.alias + ' ON ' + query.join[i].condition, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
					}
					else
					{
						querystring += Abstract_Connector.escape_columns( ( query.join[i].type == 'inner' ? ' INNER' : ' LEFT' ) + ' JOIN ' + query.join[i].table + ( query.join[i].table_alias ? ' ' + query.join[i].table_alias : '' ) + ' ON ' + query.join[i].condition, ORACLE_Connector.escape_column, ORACLE_Connector.transform_function );
					}
				}
			}
			...
			if( query.set )
			{
				...
				if( query.operation === 'update' && query.hasOwnProperty( 'update_with_where' ) && query.update_with_where )
				{
					...
					for( let i = 0; i < columns.length; ++i )
					{
						if( typeof query.data[0][columns[i]] !== 'undefined' )
						{
							set += ( i ? ', ' : '' ) + ORACLE_Connector.escape_column(columns[i]) + ' = :value';
						}
					}
			...
			if( !query.offset && query.limit && query.operation !== 'insert' )
			{
				querystring = 'SELECT * FROM ('+querystring+') WHERE ROWNUM <= '+ query.limit;
			}
			else if( query.offset && query.limit && query.operation !== 'insert' )
			{
				querystring = 'SELECT * FROM ( SELECT a.*, ROWNUM '+this.escape_column('__RNUM__')+' FROM ( '+querystring+') a  WHERE ROWNUM <= '+query.limit + query.offset + ' ) WHERE '+this.escape_column('__RNUM__')+'  > '+ query.limit;
			}
			else if( query.offset && query.operation !== 'insert' )
			{
				querystring = 'SELECT * FROM ( SELECT a.*, ROWNUM '+this.escape_column('__RNUM__')+' FROM ( '+querystring+') a ) WHERE '+this.escape_column('__RNUM__')+'  > '+ query.offset;
			}
		...
		this.execute = function( query, callback, indexes = null, auto_increment = null )
		{
			...
			if( typeof query == 'object' )
			{
				...
				if( operation === 'insert' )
				{
					...
					for( let i = 0; i < query.data.length; i++ )
					{
						...
						{
							let i_value = query.data[i][ column_name ];

							if( i_value === null || typeof i_value === 'undefined' ){ i_value = ''; }
							else if( !i_value && i_value !== 0 ){ i_value = ''; }
							else if( i_value instanceof Buffer ) { i_value = i_value.toString('hex'); }
							else if( typeof i_value === 'object' ){ i_value = JSON.stringify( i_value ); }
							else
							{
								i_value = query.data[i][ column_name ].toString();

								if( i_value.indexOf('&__escaped__:') === 0 ){ i_value = i_value.substring('&__escaped__:'.length+1, i_value.length - 1 ); }
							}
					...
					if( query.hasOwnProperty('tables') && query.tables.hasOwnProperty( query.table ) && query.tables[ query.table ].columns )
					{
						for( let j = 0; j < query.columns.length; ++j )
						{
							if( query.tables[ query.table ].columns.hasOwnProperty( query.columns[j] ) )
							{
								...
								if( type === 'CLOB' ){ bind = { dir: oracle_sql.BIND_IN, type: oracle_sql.CLOB }; }
								bind.maxSize = 32767;
			...
			return new TimedPromise( async ( resolve, reject, remaining_ms ) =>
			{
				...
				if( insertData ){ result[ 'insertData' ] = insertData; }

				emit( 'before-query', query );

				if( oracle_connections )
				{
					//const sessionTagNeeded = Math.random() > 0.5 ? "NLS_SORT=BINARY_AI" : "NLS_COMP='LINGUISTIC'";
					...
					await oracle_connections.getConnection(  async (err, conn) =>
					{
						if( !err )
						{
							...
							else if( query.match(/SELECT/i ) && query.match(/LIKE/i ) )
							{
								await conn.execute( 'ALTER SESSION SET NLS_SORT=BINARY_AI NLS_COMP=\'LINGUISTIC\'', {}, { outFormat: oracle_sql.OBJECT, autoCommit: true, extendedMetaData : true } , BIND_IF_FLOW( async (err, rows) =>
								{
						...
						else
						{
							...
							result.inserted_id   = null,
							result.inserted_ids  = [],
							result.changed_id    = null,
							result.changed_ids   = [],
							result.row           = null,
							result.rows          = [],
							result.query         = query
...
WHERE table_name = 'MYTABLE'*/
		...
		this.create_table = function( table, name = null, database = null, options = [] )
		{
			...
			let querystring = 'CREATE TABLE '  + Abstract_Connector.escape_columns( ( database ? database + '.' : '' ) + name, ORACLE_Connector.escape_column, null );
		...
		{
			return 'BEGIN '+
				'EXECUTE IMMEDIATE ' + this.escape_value( 'BEGIN EXECUTE IMMEDIATE ' + this.escape_value( 'DROP TABLE '+Abstract_Connector.escape_columns( ( database ? database + '.' : '' ) + table, ORACLE_Connector.escape_column, null )+' CASCADE CONSTRAINTS' ) + '; EXCEPTION WHEN OTHERS THEN NULL; END;' )+';'+
		...
		function shortName( word, max_length, parse_by = '_' )
		{
			...
			word.split( parse_by ).forEach( word_part => { if( word_part ) { short += capitalize( word_part.substring( 0, Math.min( word_part.length, parse_length ))); }});
