...
class Query
{
  ...
  describe_table( option = null )
  {
    return new TimedPromise( async( resolve, reject, remaining_ms ) =>
    {
      ...
      {
        table_columns = await this.connector.describe_columns( this.query.table );
      }

      if( !option || option === 'indexes' )
      {
        table_indexes = await this.connector.show_table_index( this.query.table );
      }
      ...
      {
        this.tables = { [this.query.table]: { columns: ( table_columns ? table_columns.columns : null ), indexes: ( table_indexes ? table_indexes.indexes : null ) } };
        resolve({ ok: true });
      }
    });
  }
