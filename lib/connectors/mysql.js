	...
	return new( function()
	{
		...
		function checkConnect( oneTime = false )
		{
			//if( !disconnected )
			{
				...
				if( my_connections )
				{
					my_connections.query('SELECT \'connected\' FROM DUAL', BIND_IF_FLOW( async ( err, rows ) =>
					{
		...
		this.select = function( query, callback )
		{
			...
			return new TimedPromise( async ( resolve, reject, remaining_ms ) =>
			{
				...
				this.emit( 'before-query', query );
				my_connections.query( query, BIND_IF_FLOW(( err, rows ) =>
				{
