...
module.exports = class Query
{
	...
	where( condition, data = null )
	{
		if( condition )
		{
			...
			{
				condition = expandWhere(condition, this.connector.escape_value );
			}
	...
	execute()
	{
		...
		if( this.query.table )
		{
			return new TimedPromise( async( resolve, reject, remaining_ms ) =>
			{
				...
				this.connector.select( this.query ).then( ( result ) =>
				{
	...
	get_union( alias = null )
	{
		return new TimedPromise( async( resolve, reject, remaining_ms ) =>
		{
			if( this.query.union && this.query.union.length )
			{
				...
				if( alias )
				{
					resolve( '( ' + this.connector.build( this.query ) + ' ) ' + alias + ' ' );
				}
				else
				{
					resolve( this.connector.build( this.query ));
				}
			}
	...
	async update( set, data = null ) // TODO v set metode ak stlpec pre data nie je definovany tak dat len nazonv stlpca WHEN condition THEN COLUMN - poskusat ci pojde vsade
	{
		...
		if( this.query.table )
		{
			...
			if( set )
			{
				...
				if( !this.tables )
				{
					let described = await this.describe_table();
				...
				if( data && Array.isArray(data) )
				{
					if( this.tables )
					{
						...
						//	this.query.update_with_where = false;  //TODO doriesit
				...
				if( !error )
				{
					let indexes = this._get_main_indexes(), columns = this._get_all_table_columns();

					return this.connector.update( this.query, indexes, columns ).then( ( result ) =>
					{
						/*
						if( result.ok && result.changed_rows > result.changed_ids.length )
						{
	...
	_get_all_table_columns( columns = [] )
	{
		if( this.tables[this.query.table].columns ){ columns = Object.keys( this.tables[this.query.table].columns ); }
