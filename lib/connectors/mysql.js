	...
	return new( function()
	{
		...
		this.build = function( query )
		{
			...
			if( query.hasOwnProperty( 'union' ) )  //todo if tables is defined, then can check if is column count equal
			{
				...
				else { query.table = '( '+ unions.join( ' UNION ' ) +' ) ' + ( query.alias ? query.alias : '_union_'+ Math.ceil( Math.random() * 10000000 ) ); };
			}
		...
		this.build_wo_values = function( query )
		{
			...
			if( query.hasOwnProperty( 'union' ) )  //todo if tables is defined, then can check if is column count equal
			{
				...
				let ia_uses = 0;
				let unions = [], inner_alias = '_union_' ;
		...
		this.getTablesQuery = function( db_name )
		{
			return 'SHOW TABLES' + ( db_name ? ' IN ' + this.escape_column( db_name ) : '' );
		};
		...
		this.describe_columns = function( data )
		{
			return new TimedPromise( async ( resolve, reject, remaining_ms ) =>
			{
				...
				data.forEach( column => {
					...
					if( match )
					{
						columns[ name ].type += ':' + match[1].replace(/'/g, '');
					}
					...
					if( column['Collation'] && column['Collation'] !== default_charset )
					{
		...
		this.get_duplicate_table_query = function( old_name, new_name )
		{
			return 'CREATE TABLE '+ this.escape_column( new_name ) +' LIKE '+ this.escape_column( old_name ) +'';
		}

		this.get_duplicate_table_data_query = function( old_name, new_name )
		{
			return 'INSERT INTO '+ this.escape_column( new_name ) +' SELECT * FROM '+ this.escape_column( old_name ) +'';
		}
		...
		this.create_column = function( columnData )
		{
			...
			if( columnData )
			{
				if( columnData.type )
				{
					...
					else if( columnData.encoding )
					{
						column += ' CHARACTER SET '+ columnData.encoding +' COLLATE '+ columnData.encoding + '_' + ( columnData.collate ? columnData.collate : 'general_ci' );
					}
		...
		this.create_database_query = function( database, options = {} )
		{
			...
			queries.push( 'CREATE DATABASE ' + ( options.unsafe ? '' : 'IF NOT EXISTS ' ) + this.escape_column(database) + ' DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci' );
			if( options.use ){ queries.push( 'USE ' + this.escape_column(database) + '' ); }

			return queries.join('; ');
		}

		this.drop_database_query = function( database, options = {} )
		{
			return 'DROP DATABASE ' + ( options.unsafe ? '' : 'IF EXISTS ' ) + this.escape_column(database);
		}
